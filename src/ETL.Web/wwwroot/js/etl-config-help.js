(function () {
    ...
    if (!sourceSelect || !destinationSelect || !sourceOutput || !destinationOutput) {
        return;
    }
    ...
    const destinationExamples = {
        ...
        "2": {
            connectionString: "Host=localhost;Port=5432;Database=etlprojectdb;Username=postgres;Password=1234",
    ...
    function renderExamples() {
        const sourceJson = sourceExamples[sourceSelect.value] ?? sourceExamples["1"];
        const destinationJson = destinationExamples[destinationSelect.value] ?? destinationExamples["2"];
